...
<script>
    ...
    function render_results(data){
        ...
        jQuery.each(data.hits, function(k, v) {
            ...
            if(v.multi_img) {
                ...
                image_item += '<div class="item upload'+(v.exists?' exists':'')+'" data-title="'+v.title+'" data-url="'+v.largeImageURL+'" data-invnr="'+v.inv_nr+'" data-artist="'+v.name_artist+'" data-import_source_id="'+v.import_source_id+'" data-image_id="'+v.imageId+'" data-import_inventory_id="'+v.import_inventory_id+'" data-caption="'+v.caption+v.photocredit+'" data-w="'+v.webformatWidth+'" data-h="'+v.webformatHeight+'">'
                    +'<img src="'+v.previewURL.replace('_150', '_340')+'" title="#'+v.inv_nr+'" alt="#'+v.inv_nr+'">'
                    +'<div class="dlimg">'
                        +'<img src="<?= plugin_dir_url(__FILE__).'../assets/img/round-cloud_download-24px.svg' ?>">'
                        +'<div class="caption">'+v.title+'</div>'
    ...
    jQuery("#wrklst_results").on('click', '.upload:not(.doneuploading)', function() {
        ...
        {
            jQuery(this).addClass('uploading').find('.dlimg img').replaceWith('<img src="<?= plugin_dir_url(__FILE__).'../assets/img/baseline-autorenew-24px.svg' ?>" class="loading-rotator" style="height:80px !important">');
            var that = jQuery(this);
            jQuery.post('.', {
                wrklst_upload: "1",
                image_url: jQuery(this).data('url'),
                image_caption: jQuery(this).data('caption'),
                title: jQuery(this).data('title'),
                invnr: jQuery(this).data('invnr'),
                artist: jQuery(this).data('artist'),
                import_source_id: jQuery(this).data('import_source_id'),
                image_id: jQuery(this).data('image_id'),
                import_inventory_id: jQuery(this).data('import_inventory_id'),
