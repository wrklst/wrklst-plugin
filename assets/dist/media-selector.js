var WrkLstMedia = (() => {
  ...
  // assets/src/media-selector.js
  var media_selector_exports = {};
  __export(media_selector_exports, {
    default: () => media_selector_default
  });
...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  module_default.data("wrkLstMediaSelector", (wrklstNonce) => ({
    // State
    loading: false,
    works: [],
    inventories: [],
    ...
    async loadInventories() {
      const self = this;
      window.WrkLstAjax.getInventories(wrklstNonce, function(data2) {
        if (data2 && data2.inventories) {
          self.inventories = data2.inventories;
        ...
        console.error("Failed to load inventories:", error2);
        self.fetchWorks();
      });
    },
    ...
    async fetchWorks() {
      ...
      if (this.lastCall === callKey || this.loading)
        return;
      this.loading = true;
      ...
      window.WrkLstAjax.getInventoryItems({
        ...
        self.loading = false;
        if (data2 && data2.hits) {
          self.totalHits = data2.totalHits || 0;
          const processedWorks = data2.hits.map((work) => {
        ...
        self.loading = false;
        console.error("Failed to load works:", error2);
      });
    },
    // Handle scroll
    handleScroll(event) {
    ...
    selectImage(work, img = null) {
      ...
      const imageUrl = img ? originalWork.imgs && originalWork.imgs.find((i) => i.id === img.id)?.url_full || img.url_full : originalWork.url_full;
      ...
      window.WrkLstAjax.uploadImage({
        ...
        if (originalWork) {
          if (img && originalWork.imgs) {
            ...
            if (originalImg)
              originalImg.exists = 1;
          } else {
            originalWork.exists = 1;
          }
        }
        self.works = [...self.works];
        const attachment = wp.media.model.Attachment.create({ id: data2.id });
        attachment.fetch();
        wp.media.frame.state().get("selection").add(attachment);
        wp.media.frame.content.mode("browse");
      }, function(error2) {
        self.uploadingIds = self.uploadingIds.filter((id) => id !== uploadId);
        alert("Failed to upload image: " + (error2.message || "Unknown error"));
      });
    },
    // Build caption
    buildCaption(work) {
  ...
  var media_selector_default = module_default;
  return __toCommonJS(media_selector_exports);
})();
